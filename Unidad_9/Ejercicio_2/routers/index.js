...
//Parametros url
routerBackend.get("/:titulo", (req, res) => {
...
// Routers Animales
const routerAnimales = express.Router();
app.use("/api/animales", routerAnimales);
const routerAereos = express.Router();
routerAnimales.use("/aereos", routerAereos);
const routerTerrestres = express.Router();
routerAnimales.use("/terrestres", routerTerrestres);
const routerAcuaticos = express.Router();
routerAnimales.use("/acuaticos", routerAcuaticos);

// Información de todos los animales
routerAnimales.get("/", (req, res) => {
  res.send(JSON.stringify(infoAnimales));
});

// Información solo de los animales aéreos
routerAereos.get("/", (req, res) => {
  res.send(JSON.stringify(infoAnimales.aereos));
});

// Información solo de los animales terrestres
routerTerrestres.get("/", (req, res) => {
  res.send(JSON.stringify(infoAnimales.terrestres));
});

// Información solo de los animales acuáticos
routerAcuaticos.get("/", (req, res) => {
  res.send(JSON.stringify(infoAnimales.acuaticos));
});

// Parámetros URL
routerAereos.get("/:nombre", (req, res) => {
  ...
  if (animalesFiltrados.length === 0) {
    return res
      .status(404)
      .send(`No se encontraron animales aéreos con el nombre ${nombre}`);
  }

  // Parámetros query (?ordenar=nivel)
  if (req.query.ordenar === "nivel") {
    return res.send(
      JSON.stringify(
  ...
  // Parámetros query (?ordenar=id)
  if (req.query.ordenar === "id") {
    return res.send(
      JSON.stringify(animalesFiltrados.sort((a, b) => b.id - a.id))
    );
  }

  res.send(JSON.stringify(animalesFiltrados));
});
